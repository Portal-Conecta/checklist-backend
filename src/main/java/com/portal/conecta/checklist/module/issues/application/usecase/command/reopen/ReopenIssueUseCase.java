package com.portal.conecta.checklist.module.issues.application.usecase.command.reopen;
 ...
 *
 * <p>Apenas perfis gerenciais (SENAI, WEG) podem rejeitar uma resolucao e
 * reabrir a pendencia. A transicao e validada pelo dominio — qualquer outro status resulta em
...
public class ReopenIssueUseCase {
    ...
    public ChecklistIssue execute(UUID issueId) {
        if (!contextProvider.getRequestContext().canValidateOrReopenIssues()) {
            throw new AccessDeniedException("Apenas perfis gerenciais podem reabrir pendencias.");
        }
